...
- **The new protocol should be described, not just written.**
  [ADR-0004](0004-the-client-api-is-described-in-fogs-own-openapi-document.md)
  argues that the replacement operations — and the `hostAuth` scheme this ADR's
  credential defines — belong in FOG's own OpenAPI document, so the endpoint's
  HTTP layer can be generated by the pipeline `fog-sdk` already runs. That is a
  separate decision and does not block this one, but if the protocol is being
  rewritten once, it is worth rewriting into something a machine can read.

## References

- [ADR-0004](0004-the-client-api-is-described-in-fogs-own-openapi-document.md) — describing the resulting protocol, and where `hostAuth` would be declared
